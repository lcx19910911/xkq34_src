namespace Hidistro.UI.SaleSystem.CodeBehind
{
    ...
    public class StoreCardCreater
    {
        ...
        public bool CreadCard(out string imgUrl)
        {
            ...
            imgUrl = "生成失败";
            Bitmap qRimg = new QRCodeEncoder { QRCodeEncodeMode = QRCodeEncoder.ENCODE_MODE.BYTE, QRCodeScale = 8, QRCodeVersion = 8, QRCodeErrorCorrect = QRCodeEncoder.ERROR_CORRECTION.M }.Encode(this.CodeUrl);
            ...
            qRimg.Dispose();
            bitmap3.Save(Globals.MapPath(string.Concat(new object[] { Globals.GetStoragePath(), "/DistributorCards/StoreCard", this.userId, ".jpg" })), ImageFormat.Jpeg);
            bitmap3.Dispose();
            flag = true;
            imgUrl = string.Concat(new object[] { Globals.GetStoragePath(), "/DistributorCards/StoreCard", this.userId, ".jpg" });
            return flag;
        }
        ...
        private Bitmap getNetImg(string imgUrl)
        {
            try
            {
                ...
                Stream responseStream = WebRequest.Create(imgUrl + "?aid=&" + random.NextDouble()).GetResponse().GetResponseStream();
