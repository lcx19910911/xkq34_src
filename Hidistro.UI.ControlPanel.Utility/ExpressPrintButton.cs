        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            if ((isUserExpressTemplates != null) && (isUserExpressTemplates.Rows.Count > 0))
            {
                ...
                else if (!string.IsNullOrEmpty(this.Page.Request.QueryString["PurchaseOrderId"]))
                {
                    foreach (DataRow row in isUserExpressTemplates.Rows)
                    {
                        builder.AppendFormat("<a href=\"flex/print.html?ShipperId={0}&PurchaseOrderId={1}&XmlFile={2}\" style=\"margin-right:10px;\" >{3}</a> ", new object[] { this.ShipperId, this.Page.Request.QueryString["PurchaseOrderId"], row["XmlFile"], row["ExpressName"] });
                    }
                }
                builder.Append("</div>");
