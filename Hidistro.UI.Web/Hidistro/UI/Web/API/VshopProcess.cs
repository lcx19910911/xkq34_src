...
namespace Hidistro.UI.Web.API
{
    public class VshopProcess : IHttpHandler, IRequiresSessionState
    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    DistributorsBrower.DistributorGradeChange(distributors, "", distributors.DistriGradeId);
        ...
        //            {
        //                CS$<>9__CachedAnonymousMethodDelegate5 = new Func<JToken, int>(null, (IntPtr) <AddDistributorProducts>b__4);
        //            }
        //            DistributorsBrower.AddDistributorProductId(Enumerable.Select<JToken, int>(source.Values(), CS$<>9__CachedAnonymousMethodDelegate5).ToList<int>());
        //        }
        //    }
        //    context.Response.Write("{\"success\":\"true\",\"msg\":\"保存成功\"}");
        //    context.Response.End();
                ...
                if ((int.TryParse(str2, out result) && (result > 0)) && (oneyuanTaoInfoById != null))
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            info.ActivityId = str;
                            info.UserId = Globals.GetCurrentMemberUserId();
                            info.SkuId = str3;
        ...
        private void AddProductConsultations(HttpContext context)
        {
            ...
            if (ProductBrowser.InsertProductConsultation(productConsultation))
            {
                ...
                try
                {
                    ...
                    {
                        Messenger.SendWeiXinMsg_ProductAsk(productBaseInfo.ProductName, "", productConsultation.ConsultationText);
                    }
                }
                catch (Exception)
                {
        ...
        private void GetMyDistributors(HttpContext context)
        {
            ...
            int total = 0;
            DataTable downDistributors = DistributorsBrower.GetDownDistributors(query, out total);
            string myDistributorsHtml = this.GetMyDistributorsHtml(downDistributors);
            string s = string.Empty;
            if (downDistributors.Rows.Count > 0)
            {
                s = string.Concat(new object[] { "{\"success\":\"true\",\"rowtotal\":\"", downDistributors.Rows.Count, "\",\"total\":\"", total, "\",\"lihtml\":\"", myDistributorsHtml, "\"}" });
            }
            else
            {
                s = "{\"success\":\"false\"}";
        ...
        public string GetMyDistributorsHtml(DataTable dt)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    builder.Append("<p><a href='ChirldrenDistributorDetials.aspx?distributorId=" + dt.Rows[i]["UserId"].ToString() + "'><span class='colorc'>销售总额：</span><span class='colorg'>" + (string.IsNullOrEmpty(dt.Rows[i]["OrderTotal"].ToString()) ? "0.00" : string.Format("{0:F2}", dt.Rows[i]["OrderTotal"].ToString())) + "</span></a></p>");
                    builder.Append("</div>");
                    builder.Append("<div class='right'>");
                    builder.Append("<p><a href='ChirldrenDistributorStores.aspx?UserId=" + dt.Rows[i]["UserId"].ToString() + "'><span class='colorc'>下级分店：</span><span class='colorg'>" + dt.Rows[i]["disTotal"].ToString() + " 家</span></a></p>");
                    builder.Append("<p><span class='colorc'>下级会员：</span>" + dt.Rows[i]["MemberTotal"].ToString() + " 位</p>");
                    builder.Append("<p><a href='ChirldrenDistributorDetials.aspx?distributorId=" + dt.Rows[i]["UserId"].ToString() + "'><span class='colorc'>贡献佣金：</span><span class='colorg'>￥" + (string.IsNullOrEmpty(dt.Rows[i]["CommTotal"].ToString()) ? "0.00" : string.Format("{0:F2}", dt.Rows[i]["CommTotal"].ToString())) + "</span></a></p>");
                    builder.Append("</div>");
                    builder.Append("</div>");
                    builder.Append("</div>");
        ...
        private void GetMyMember(HttpContext context)
        {
            ...
            int pageSize = 10;
            int total = 0;
            DataTable dt = MemberProcessor.GetMembersByUserId(referralUserId, pageIndex, pageSize, out total);
        ...
        private string GetMyMemberHtml(DataTable dt)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    builder.Append("<p><span class='colorc'>消费金额：</span><span class='colorg'>￥" + (string.IsNullOrEmpty(dt.Rows[i]["OrdersTotal"].ToString()) ? "0" : string.Format("{0:F}", dt.Rows[i]["OrdersTotal"].ToString())) + "</span></p>");
                    builder.Append("</div>");
                    builder.Append("</div>");
                    builder.Append("</div>");
                    builder.Append("<span class='left radius'></span>");
        ...
        private void GetSecondDistributors(HttpContext context)
        {
            ...
            if (downDistributors.Rows.Count > 0)
            {
                s = string.Concat(new object[] { "{\"success\":\"true\",\"rowtotal\":\"", downDistributors.Rows.Count, "\",\"total\":\"", total, "\",\"lihtml\":\"", secondDistributorsHtml, "\"}" });
            }
            else
            {
                s = "{\"success\":\"false\"}";
        ...
        public string ordersummit(ShoppingCartInfo cart, HttpContext context, string remark, int shippingId, string couponCode, string selectCouponValue, string shippingTypeinfo, bool summittype, string OrderMarking, IList<ShoppingCartItemInfo> ItemInfo, int PointExchange, int bargainDetialId, out string ActivitiesIds)
        {
            ...
            if ((order.DiscountAmount > 0M) || (couponId > 0))
            {
                        ...
                        case 1:
                            {
                                ...
                                currentDistributorId = distributorInfo.UserId;
            ...
            if (int.TryParse(shippingTypeinfo, out result))
            {
                ...
                if (result > 0)
                {
                    ...
                    if (defaultView.Count > 0)
                    {
                        Dictionary<int, ShoppingCartItemInfo> dictionary2 = new Dictionary<int, ShoppingCartItemInfo>();
                        foreach (ShoppingCartItemInfo info7 in cart.LineItems)
                        {
                            if (!dictionary2.ContainsKey(info7.FreightTemplateId))
                            {
                                info7.SumSubTotal = info7.SubTotal;
                                info7.CubicMeter *= info7.Quantity;
                                info7.FreightWeight *= info7.Quantity;
                                dictionary2.Add(info7.FreightTemplateId, info7);
                            }
                            else
                            {
                                ShoppingCartItemInfo local1 = dictionary2[info7.FreightTemplateId];
                                local1.SumSubTotal += info7.SubTotal;
                                ShoppingCartItemInfo local2 = dictionary2[info7.FreightTemplateId];
                                local2.FreightWeight += info7.FreightWeight * info7.Quantity;
                                ShoppingCartItemInfo local3 = dictionary2[info7.FreightTemplateId];
                                local3.CubicMeter += info7.CubicMeter * info7.Quantity;
                                ShoppingCartItemInfo local4 = dictionary2[info7.FreightTemplateId];
                                local4.Quantity += info7.Quantity;
                            }
                        }
                        cart.LineItems.Clear();
                        foreach (KeyValuePair<int, ShoppingCartItemInfo> pair2 in dictionary2)
                        {
                            cart.LineItems.Add(pair2.Value);
                        }
                        foreach (ShoppingCartItemInfo info8 in cart.LineItems)
                        {
                            ...
                            if (!info8.IsfreeShipping)
                            {
                                ...
                                if (((templateMessage != null) && (info8.FreightTemplateId > 0)) && !templateMessage.FreeShip)
                                {
                                    if (templateMessage.HasFree)
                                    {
                                        flag = this.IsFreeTemplateShipping(context.Request["Shippingcity"], info8.FreightTemplateId, result, info8);
                                    }
                                    if (!flag)
                                    {
                                        ...
                                        if (str6 != null)
                                        {
                                            ...
                                            {
                                                num7 += this.setferight(float.Parse(info8.Quantity.ToString()), float.Parse(defaultView[0]["FristNumber"].ToString()), float.Parse(defaultView[0]["FristPrice"].ToString()), float.Parse(defaultView[0]["AddNumber"].ToString()), float.Parse(defaultView[0]["AddPrice"].ToString()));
                                            }
                                        }
                                    }
                                }
                        ...
                        Label_06E7:
                            ...
                            {
                                num7 += this.setferight(float.Parse(info8.FreightWeight.ToString()), float.Parse(defaultView[0]["FristNumber"].ToString()), float.Parse(defaultView[0]["FristPrice"].ToString()), float.Parse(defaultView[0]["AddNumber"].ToString()), float.Parse(defaultView[0]["AddPrice"].ToString()));
                            }
                            continue;
                        Label_0793:
                            if (info8.CubicMeter > 0M)
                            {
                                num7 += this.setferight(float.Parse(info8.CubicMeter.ToString()), float.Parse(defaultView[0]["FristNumber"].ToString()), float.Parse(defaultView[0]["FristPrice"].ToString()), float.Parse(defaultView[0]["AddNumber"].ToString()), float.Parse(defaultView[0]["AddPrice"].ToString()));
                            }
                            continue;
                        Label_083F: ;
                    ...
                    string s = num7.ToString("F2");
                    order.AdjustedFreight = decimal.Parse(s);
                }
            }
            if (int.TryParse(context.Request["paymentType"], out num6))
            {
                order.PaymentTypeId = num6;
                switch (num6)
                {
                    case 0:
                    case -1:
                ...
                PaymentModeInfo paymentMode = ShoppingProcessor.GetPaymentMode(num6);
                if (paymentMode != null)
                {
        ...
        Label_0BC9:
            ...
            if (masterSettings.PonitToCash_Enable && (bargainDetialId == 0))
            {
                if (PointExchange > 0)
                {
                    ...
                    decimal num9 = masterSettings.PonitToCash_MaxAmount;
                    float num10 = float.Parse(PointExchange.ToString()) / float.Parse(pointToCashRate.ToString());
                    decimal total = 0M;
                    total = order.GetTotal();
                    if ((decimal.Parse(num10.ToString()) > total) || (num9 < decimal.Parse(num10.ToString())))
                    {
                        builder.Append("\"Status\":\"Error\"");
                        builder.AppendFormat(string.Concat(new object[] { ",\"ErrorMsg\":\"抵现金额不能大于应付总额", total, "元,最高抵现金额", num9, "元！\"" }), new object[0]);
                        return builder.ToString();
                    }
                    order.PointToCash = decimal.Parse(num10.ToString());
            ...
            try
            {
                decimal decimal1 = (order.RedPagerAmount + order.PointToCash) + order.DiscountAmount;
                this.SetOrderItemStatus(order, order.RedPagerAmount, order.PointToCash, order.DiscountAmount, vItemList);
        ...
        private void ProcessSubmmitorder(HttpContext context)
        {
            ...
            else
            {
                ...
                if (strArray2.Length > 0)
                {
                    ...
                    if ((num3 != 0) && (num3 > currentMember.Points))
                    {
                ...
                if (bargainDetialId > 0)
                {
                    ...
                    string str3 = BargainHelper.IsCanBuyByBarginDetailId(bargainDetialId);
                    if (str3 != "1")
                    {
                        builder.Append("\"Status\":\"Eror\",\"ErrorMsg\":\"" + str3 + ",不能提交订单！\"");
                        builder.Append("}");
                        context.Response.ContentType = "application/json";
                        context.Response.Write(builder.ToString());
                        return;
                ...
                int.TryParse(context.Request["groupbuyId"], out num6);
                int relNumber = 0;
                string remark = context.Request["remark"];
                string orderMarking = this.GenerateOrderId();
                if (((int.TryParse(context.Request["buyAmount"], out num7) && !string.IsNullOrEmpty(context.Request["productSku"])) && !string.IsNullOrEmpty(context.Request["from"])) && ((context.Request["from"] == "signBuy") || (context.Request["from"] == "groupBuy")))
                {
                    ...
                    if (context.Request["from"] == "signBuy")
                    {
                        ...
                        if (bargainDetialId > 0)
                        {
                            if (BargainHelper.UpdateNumberById(bargainDetialId, num7, out relNumber))
                            {
                        ...
                        else
                        {
                            ...
                            if (id > 0)
                            {
                                ...
                                if (flag3)
                                {
                                    int num11 = ShoppingCartProcessor.GetLimitedTimeDiscountUsedNum(id, productSkuId, 0, currentMember.UserId, false);
                                    if ((discountInfo.LimitNumber > 0) && (num7 > (discountInfo.LimitNumber - num11)))
                                    {
                                        ...
                                        {
                                            buyAmount = discountInfo.LimitNumber - num11;
                                        }
                                        else
                                        {
                                            buyAmount = 0;
                        ...
                        else
                        {
                            ...
                            {
                                string str9 = this.ordersummit(info3, context, remark, shippingId, couponCode, strArray[0], context.Request["shippingType"], false, orderMarking, info3.LineItems, int.Parse(strArray2[0]), bargainDetialId, out activitiesIds);
                                builder.Append(str9);
                                str4 = activitiesIds + ",";
                            }
                        }
                ...
                else
                {
                    ...
                    foreach (ShoppingCartInfo info4 in orderSummitCart)
                    {
                        foreach (ShoppingCartItemInfo info5 in info4.LineItems)
                        {
                            if (info5.Type == 1)
                            {
                                num13 += info5.PointNumber;
                            }
                        }
                    }
                    if (num13 > currentMember.Points)
                    {
                        builder.Append("\"Status\":\"Eror\",\"ErrorMsg\":\"您当前积分不足！\"");
                    }
                    else
                    {
                        ...
                        {
                            this.ordersummit(info6, context, strArray4[index], shippingId, couponCode, strArray[index], strArray3[index], true, orderMarking, info6.LineItems, int.Parse(strArray2[index]), 0, out activitiesIds);
                            str4 = str4 + activitiesIds + ",";
                            index++;
                        }
                ...
                if (!string.IsNullOrEmpty(str4))
                {
                    foreach (string str10 in str4.Substring(0, str4.Length - 1).Split(new char[] { ',' }))
                    {
                        if (!string.IsNullOrEmpty(str10) && (int.Parse(str10) > 0))
                        {
                            int activitiesId = ActivityHelper.GetHishop_Activities(int.Parse(str10));
                            int userId = currentMember.UserId;
        ...
        public void RequestReturn(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(refundInfo.Account.Trim()))
            {
                if (!ShoppingProcessor.GetReturnInfo(refundInfo.UserId, refundInfo.OrderId, refundInfo.ProductId, refundInfo.SkuId))
                {
                    ...
                    {
                        if (ShoppingProcessor.UpdateOrderGoodStatu(refundInfo.OrderId, refundInfo.SkuId, orderItemsStatus))
                        {
                            try
                            {
                                this.myNotifier.updateAction = UpdateAction.OrderUpdate;
