        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            else
            {
                try
                {
                    ...
                    writer.WriteLine(DateTime.Now + "-1");
                    writer.Flush();
                    writer.Close();
                    string senderId = "";
                    string receiverId = "";
                    ...
                    if ((obj2["type"] != null) && (obj2["type"].ToString() == "event"))
                    {
                        ...
                        if (obj4["subtype"].ToString().Trim() == "click")
                        {
                            StreamWriter writer2 = File.AppendText(context.Server.MapPath("wberror.txt"));
                            writer2.WriteLine(masterSettings.SubscribeReply ? "是" : "否");
                            writer2.WriteLine(DateTime.Now + "-6");
                            writer2.Flush();
                            writer2.Close();
                        }
                        else if (masterSettings.SubscribeReply && (obj4["subtype"].ToString().Trim() == "subscribe"))
                        {
                            senderId = obj2["receiver_id"].ToString();
                            receiverId = obj2["sender_id"].ToString();
                    ...
                    writer3.WriteLine(str5);
                    writer3.WriteLine(DateTime.Now + "-2");
                    writer3.Flush();
                    writer3.Close();
                }
                catch (Exception exception)
                {
                    StreamWriter writer4 = File.AppendText(context.Server.MapPath("wberror.txt"));
                    writer4.WriteLine(exception.Message);
                    writer4.WriteLine(exception.StackTrace);
                    writer4.WriteLine(DateTime.Now + "-3");
                    writer4.Flush();
                    writer4.Close();
                }
            }
