...
namespace Hidistro.UI.Web.Admin.AliFuwu
{
    public class ReplyOnKey : AdminPage
    {
        ...
        protected string GetTitleShow(object messagetypename, object articleid, object responseid)
        {
            ...
            if (!(str2 == "多图文"))
            {
                if (str2 == "单图文")
                {
                    ...
                    NewsReplyInfo info6 = AliFuwuReplyHelper.GetReply(id) as NewsReplyInfo;
                    if (info6 == null)
                    {
                        return str;
                    }
                ...
                if (str2 == "文本")
                {
                    TextReplyInfo info8 = AliFuwuReplyHelper.GetReply(id) as TextReplyInfo;
                    if (info8 != null)
                    {
            ...
            NewsReplyInfo reply = AliFuwuReplyHelper.GetReply(id) as NewsReplyInfo;
            if (reply == null)
            {
                return str;
            }
        ...
        protected void rptList_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            {
                AliFuwuReplyHelper.DeleteReply(id);
                this.BindArticleCategory();
                this.ShowMsg("删除成功！", true);
            }
            else if (e.CommandName == "Release")
            {
                AliFuwuReplyHelper.UpdateReplyRelease(id);
                base.Response.Redirect(HttpContext.Current.Request.Url.ToString(), true);
            }
            else if (e.CommandName == "Edit")
            {
                Hidistro.Entities.VShop.ReplyInfo reply = AliFuwuReplyHelper.GetReply(id);
                if (reply != null)
                {
