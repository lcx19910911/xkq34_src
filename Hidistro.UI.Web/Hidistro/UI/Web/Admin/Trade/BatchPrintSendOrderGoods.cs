namespace Hidistro.UI.Web.Admin.Trade
{
    ...
    public class BatchPrintSendOrderGoods : AdminPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                foreach (OrderInfo info in this.GetPrintData(str))
                {
                    ...
                    builder.Append("<table><col class=\"col-1\" /><col class=\"col-3\" /><col class=\"col-3\" /><col class=\"col-3\" /><col class=\"col-4\" /><col class=\"col-5\" /><thead><tr><th>商品信息</th><th>商家编码</th><th>单价</th><th>数量</th><th>小计</th><th>总价</th></tr></thead><tbody>");
                    ...
                    if (lineItems != null)
                    {
                        ...
                        foreach (string str2 in lineItems.Keys)
                        {
                            ...
                            {
                                str3 = "(已退货，金额￥" + info2.ReturnMoney.ToString("F2") + ")";
                            }
                            else if (info2.OrderItemsStatus == OrderStatus.Refunded)
                            {
                                str3 = "(已退款，金额￥" + info2.ReturnMoney.ToString("F2") + ")";
                            }
                            builder.AppendFormat("<tr><td>{0}</td>", info2.ItemDescription + str3 + (string.IsNullOrEmpty(info2.SKUContent) ? "" : ("<br>" + info2.SKUContent)));
                            builder.AppendFormat("<td style='text-align:center;'>{0}</td>", string.IsNullOrEmpty(info2.SKU) ? "-" : info2.SKU);
                    ...
                    builder.Append("</tbody></table><br class=\"clear\" /><br>");
