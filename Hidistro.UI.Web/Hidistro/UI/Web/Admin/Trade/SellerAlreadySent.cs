namespace Hidistro.UI.Web.Admin.Trade
{
    ...
    public class SellerAlreadySent : AdminPage
    {
        ...
        protected Button btnSelOrdersFinish;
                            ...
                            if (!flag)
                            {
                                if (OrderHelper.ConfirmOrderFinish(orderInfo))
                                {
                                    ...
                                    foreach (LineItemInfo info3 in orderInfo.LineItems.Values)
                                    {
                                        ...
                                        {
                                            RefundHelper.UpdateOrderGoodStatu(orderInfo.OrderId, info3.SkuId, 5);
                                        }
                                    }
        ...
        private OrderQuery GetOrderQuery()
        {
            ...
            if (this.stype == 1)
            {
                query.Gateway = "hishop.plugins.payment.podrequest";
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (Globals.RequestFormStr("isCallback"))
            {
                ...
                case "GetOrdersStates":
                {
                    ...
                    int length = allOrderID.Select("OrderStatus=" + 3).Length;
                    int num3 = allOrderID.Select("OrderStatus=" + 3 + " and Gateway = 'hishop.plugins.payment.podrequest'").Length;
                    string s = string.Concat(new object[] { "{\"type\":\"1\",\"sellalreadysentcount\":", length, ",\"count1\":", num3, "}" });
