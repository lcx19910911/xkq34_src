namespace Hidistro.UI.Web.Admin.Trade
{
    ...
    public class RecycleStationDetail : AdminPage
    {
        ...
        protected RecycleStationDetail()
            : base("m03", "ddp08")
        {
        ...
        protected void btnRefuseConfirm_Click(object sender, EventArgs e)
        {
            ...
            {
                RefundInfo refundInfo = RefundHelper.GetByOrderIdAndProductID(this.hdfOrderID.Value, productid, skuid, orderitemid);
                refundInfo.RefundId = refundInfo.ReturnsId;
                refundInfo.AdminRemark = this.txtAdminMemo.Text.Trim();
                refundInfo.HandleTime = DateTime.Now;
                refundInfo.HandleStatus = RefundInfo.Handlestatus.RefuseRefunded;
                refundInfo.Operator = Globals.GetCurrentManagerUserId().ToString();
