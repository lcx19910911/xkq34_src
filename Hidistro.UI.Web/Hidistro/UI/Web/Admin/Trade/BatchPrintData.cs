                ...
                case "getmyaddr":
                    {
                        ...
                        if (shippers.Count > 0)
                        {
                            ...
                            foreach (ShippersInfo info2 in shippers)
                            {
                                ...
                                {
                                    flag = true;
                                    builder.Append(string.Concat(new object[] { "<option value='", info2.ShipperId, "' selected='selected'>", Globals.String2Json(info2.ShipperTag), "</option>" }));
                                }
                                else
                                {
                                    builder.Append(string.Concat(new object[] { "<option value='", info2.ShipperId, "'>", Globals.String2Json(info2.ShipperTag), "</option>" }));
                                }
                            }
            ...
            else
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    for (int i = 0; i < table.Rows.Count; i++)
                    {
                        ...
                        string[] strArray = row["shippingRegion"].ToString().Split(new char[] { (char)0xff0c });
                        foreach (XmlNode node2 in node.SelectNodes("item"))
                        {
                            ...
                            switch (str4.Split(new char[] { '_' })[0])
                            {
                                case "收货人-姓名":
                                    {
                        ...
                        Label_0C90:
                            ...
                            str12 = string.Concat(new object[] { str12, "规格:", this.GetFormatTitle(row2["SKUContent"]), " 商家编码:", this.GetFormatTitle(row2["SKU"]), " 数量:", row2["ShipmentQuantity"], " " });
                            num12++;
                        Label_0D14:
                        ...
                        Label_0D32:
                            if (num2 > i)
                            {
                                for (int j = i + 1; j < (num2 + 1); j++)
                                {
                                    ...
                                    {
                                        str12 = string.Concat(new object[] { str12, "规格:", this.GetFormatTitle(row3["SKUContent"]), " 商家编码:", row3["SKU"], " 数量:", row3["ShipmentQuantity"], " " });
                                    }
                                    str12 = str12.Replace(";", "");
                                }
                            }
