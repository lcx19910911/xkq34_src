namespace Hidistro.UI.Web.Admin.Trade
{
    ...
    public class Closed : AdminPage
    {
        ...
        private OrderQuery GetOrderQuery()
        {
            ...
            if (this.stype == 1)
            {
                query.OrderItemsStatus = Hidistro.Entities.Orders.OrderStatus.Refunded;// 9;
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (Globals.RequestFormStr("isCallback"))
            {
                ...
                case "GetOrdersStates":
                    {
                        base.Response.ContentType = "application/json";
                        int length = OrderHelper.GetAllOrderID().Select("OrderStatus=" + 4).Length;
                        int countOrderIDByStatus = OrderHelper.GetCountOrderIDByStatus(Hidistro.Entities.Orders.OrderStatus.Closed, Hidistro.Entities.Orders.OrderStatus.Refunded);//(4, 9);
                        string s = string.Concat(new object[] { "{\"type\":\"1\",\"closedcount\":", length, ",\"count1\":", countOrderIDByStatus, "}" });
