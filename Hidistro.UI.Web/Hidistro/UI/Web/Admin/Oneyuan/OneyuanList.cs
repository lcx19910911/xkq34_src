        ...
        private void BindData()
        {
            ...
            if (oneyuanTao.Data != null)
            {
                ...
                foreach (DataRow row in data.Rows)
                {
                    ...
                    OneTaoState state = OneyuanTaoHelp.getOneTaoState(info);
                    OneTaoPrizeState state2 = OneyuanTaoHelp.getPrizeState(info);
                    row["ASate"] = state;
                    row["PrizeState"] = state2;
                    row["CanDel"] = 0;
                    switch (state2)
                    {
                        case OneTaoPrizeState.成功开奖:
                            row["PrizeState"] = "<span class='success'>" + state2 + "<span>";
                            break;

                        case OneTaoPrizeState.已关闭:
                            row["PrizeState"] = "<span class='normal'>" + state2 + "<span>";
                            break;

                        case OneTaoPrizeState.待退款:
                            row["PrizeState"] = "<span class='green'>" + state2 + "<span>";
                            break;

                        default:
                            row["PrizeState"] = "<span class='errcss'>" + state2 + "<span>";
                            break;
                    }
                    string str = "<a class=\"btn btn-xs btn-primary\" onclick=\"AView('" + row["ActivityId"] + "')\" >查看</a> ";
                    if ((state == OneTaoState.进行中) || (state == OneTaoState.未开始))
                    {
                        object obj2 = str;
                        str = string.Concat(new object[] { obj2, "<a class=\"btn btn-xs btn-primary\" onclick=\"AEdit('", row["ActivityId"], "')\"  >编辑</a> " });
                    }
                    switch (state)
                    {
                        ...
                        {
                            object obj3 = str;
                            str = string.Concat(new object[] { obj3, "<a class=\"btn btn-xs btn-success\" onclick=\"AStart('", row["ActivityId"], "')\"  >开启</a> " });
                            break;
                        }
                        case OneTaoState.进行中:
                        {
                            object obj4 = str;
                            str = string.Concat(new object[] { obj4, "<a class=\"btn btn-xs btn-danger\" onclick=\"AEnd('", row["ActivityId"], "','", row["FinishedNum"], "','", row["ReachType"], "','", row["ReachNum"], "')\"  >结束</a> " });
                            break;
                        }
                    }
                    if (((state == OneTaoState.已结束) && (((int) row["FinishedNum"]) == 0)) || (state == OneTaoState.未开始))
                    {
                        object obj5 = str;
                        str = string.Concat(new object[] { obj5, "<a class=\"btn btn-xs btn-danger\" onclick=\"ADel('", row["ActivityId"], "')\" >删除</a> " });
                        row["CanDel"] = 1;
                    }
                    if (state == OneTaoState.开奖失败)
                    {
                        object obj6 = str;
                        str = string.Concat(new object[] { obj6, "<a class=\"btn btn-xs btn-danger\" onclick=\"BatchRefund('", row["ActivityId"], "')\" >批量退款</a> " });
                    }
                    row["ActionBtn"] = str;
                }
