namespace Hidistro.UI.Web.Admin.promotion
{
    ...
    public class GetMemberGradesHandler : IHttpHandler
    {
        ...
        private void GetSearchUserCount(HttpContext context)
        {
            ...
            string gradeIds = context.Request["gId"];
            if (string.IsNullOrWhiteSpace(gradeIds))
            {
                context.Response.Write(JsonConvert.SerializeObject(type));
            }
            else
            {
                string s = context.Request["rId"];
                ...
                {
                    string customGroup = context.Request["cGroup"];
                    ...
                    else
                    {
                        ...
                        try
                        {
                            int num = CouponHelper.GetMemeberNumBySearch(gradeIds, s, str3, str4, int.Parse(str5), customGroup);
                            var type2 = new {
                                status = "ok",
                                msg = "",
                                count = num
        ...
        {
            context.Response.ContentType = "application/json";
            string distributorname = context.Request["storeName"];
            if (!string.IsNullOrWhiteSpace(distributorname))
            {
                int num = MemberHelper.IsExiteDistributorNames(distributorname);
                if (num > 0)
                {
        ...
        private void GetUserIdByNiCh(HttpContext context)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        context.Response.Write(JsonConvert.SerializeObject(new { status = "ok", msg = "", ids = string.Join<int>("_", list), count = list.Count }));
                    }
                }
                catch (Exception)
                {
        ...
        {
            context.Response.ContentType = "application/json";
            string userIds = context.Request["ids"];
            ...
            else
            {
                ...
                else
                {
                    try
                    {
                        if (CouponHelper.SendCouponToMemebers(result, userIds))
                        {
                            context.Response.Write(JsonConvert.SerializeObject(new { status = "ok", msg = "发送成功！" }));
                        }
                        else
