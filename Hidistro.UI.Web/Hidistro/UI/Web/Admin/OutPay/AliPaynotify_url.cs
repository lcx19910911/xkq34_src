            ...
            if (requestPost.Count > 0)
            {
                ...
                if (notify.Verify(requestPost, base.Request.Form["notify_id"], base.Request.Form["sign"]))
                {
                    ...
                    catch (Exception)
                    {
                        try
                        {
                            using (StreamWriter writer = new StreamWriter(Globals.MapPath("/Admin/OutPay/App/") + "payLog.txt", true, Encoding.UTF8))
                            {
                                writer.Write(DateTime.Now.ToString("[yyyy-MM-dd HH:mm:ss]") + "支付成功，写入数据库失败->" + base.Request.Form.ToString() + "\r\n");
                            }
                        }
                        catch (Exception)
                        {
                ...
                else
                {
                    ...
                    try
                    {
                        using (StreamWriter writer2 = new StreamWriter(Globals.MapPath("/Admin/OutPay/App/") + "payLog.txt", true, Encoding.UTF8))
                        {
                            writer2.Write(DateTime.Now.ToString("[yyyy-MM-dd HH:mm:ss]") + "支付失败->" + base.Request.Form.ToString() + "\r\n");
                        }
                    }
                    catch (Exception)
                    {
