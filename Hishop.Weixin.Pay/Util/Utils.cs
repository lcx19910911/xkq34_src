...
namespace Hishop.Weixin.Pay.Util
{
    internal class Utils
    {
        ...
        public static void WxPaylog(string log, [Optional, DefaultParameterValue("_WxPaylog.txt")] string logname)
        {
            lock (LockLog)
            {
                try
                {
                    StreamWriter writer = File.AppendText(HttpRuntime.AppDomainAppPath.ToString() + "log/" + (DateTime.Now.ToString("yyyyMMdd") + logname));
