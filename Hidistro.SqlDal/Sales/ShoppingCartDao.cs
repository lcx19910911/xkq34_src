namespace Hidistro.SqlDal.Sales
{
    ...
    public class ShoppingCartDao
    {
        ...
        public ShoppingCartItemInfo GetCartItemInfo(MemberInfo member, string skuId, int quantity, [Optional, DefaultParameterValue(0)] int type, [Optional, DefaultParameterValue(0)] int bargainDetialId, [Optional, DefaultParameterValue(0)] int limitedTimeDiscountId)
        {
            ...
            using (IDataReader reader = this.database.ExecuteReader(storedProcCommand))
            {
                ...
                if (limitedTimeDiscountId > 0)
                {
                    ...
                    {
                        info.MemberPrice = info.AdjustedPrice = info3.FinalPrice;
