namespace Hidistro.SqlDal.Store
{
    ...
    public class NoticeDao
    {
        ...
        public int GetNoticeNotReadCount(NoticeQuery query)
        {
            ...
            if (!query.IsDistributor.HasValue ? true : !query.IsDistributor.Value)
            {
                builder.Append(" AND SendTo<>1 ");
            }
            builder.AppendFormat(" AND IsPub={0} ", 1);
            if (query.IsDel.HasValue && (((nullable = query.IsDel).GetValueOrDefault() == 1) && nullable.HasValue))
            {
                str = "vw_Hishop_Notice";
        ...
        public DataTable GetNoticeNotReadDt(NoticeQuery query)
        {
            ...
            if (!(!query.IsDistributor.HasValue ? false : (!query.IsDistributor.HasValue ? true : query.IsDistributor.Value)))
            {
                builder.Append(" AND SendTo<>1 ");
            }
            builder.AppendFormat(" AND IsPub={0} ", 1);
        ...
        public DbQueryResult GetNoticeRequest(NoticeQuery query)
        {
            ...
            if (query.IsDel.HasValue && (((nullable2 = query.IsDel).GetValueOrDefault() == 1) && nullable2.HasValue))
            {
                table = "vw_Hishop_Notice";
