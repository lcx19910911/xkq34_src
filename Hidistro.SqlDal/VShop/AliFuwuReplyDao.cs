namespace Hidistro.SqlDal.VShop
{
    ...
    public class AliFuwuReplyDao
    {
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("DELETE vshop_AliFuwuReply WHERE ReplyId = @ReplyId;DELETE vshop_Message WHERE ReplyId = @ReplyId");
        ...
        public IList<Hidistro.Entities.VShop.ReplyInfo> GetReplies(ReplyType type)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select ReplyId,Keys,MatchType,ReplyType,MessageType,IsDisable,LastEditDate,LastEditor,Content,Type,ActivityId,ArticleID ");
            builder.Append(" FROM vshop_AliFuwuReply ");
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM vshop_AliFuwuReply WHERE ReplyId = @ReplyId");
            this.database.AddInParameter(sqlStringCommand, "ReplyId", DbType.Int32, id);
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT COUNT(*) FROM vshop_AliFuwuReply WHERE Keys = @Keys");
        ...
        public bool HasReplyKey(string key, int replyid)
        {
            ...
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT COUNT(*) FROM vshop_AliFuwuReply WHERE Keys = @Keys " + str);
            this.database.AddInParameter(sqlStringCommand, "Keys", DbType.String, key);
            return (Convert.ToInt32(this.database.ExecuteScalar(sqlStringCommand)) > 0);
        }
        ...
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("insert into vshop_AliFuwuReply(");
        ...
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("update vshop_AliFuwuReply set ");
        ...
        public bool UpdateReplyRelease(int id)
        {
            ...
            if (reply.IsDisable)
            {
                ...
                {
                    builder.AppendFormat("update  vshop_AliFuwuReply set IsDisable = 1 where ReplyType&{0}>0;", 4);
                }
                if ((reply.ReplyType & ReplyType.Subscribe) == ReplyType.Subscribe)
                {
                    builder.AppendFormat("update  vshop_AliFuwuReply set IsDisable = 1 where ReplyType&{0}>0;", 1);
                }
            }
            builder.Append("update vshop_AliFuwuReply set ");
            builder.Append("IsDisable=~IsDisable");
