    ...
    public class VoteDao
    {
        ...
        public VoteInfo GetVoteById(long voteId)
        {
            ...
            if (info != null)
            {
                ...
                using (reader = this.database.ExecuteReader(sqlStringCommand))
                {
                    IList<VoteItemInfo> list = ReaderConvert.ReaderToList<VoteItemInfo>(reader);
        ...
        //        if ((voteById != null) && (vote.VoteItems.Count > 0))
        //        {
        //            <>c__DisplayClass4 class3;
        //            using (IEnumerator<VoteItemInfo> enumerator = vote.VoteItems.GetEnumerator())
        //            {
        //                while (enumerator.MoveNext())
        //                {
        //                    Func<VoteItemInfo, bool> func = null;
        //                    VoteItemInfo item = enumerator.Current;
        //                    if (voteById.VoteItems.Count == 0)
        //                    {
        //                        this.CreateVoteItem(item);
        ...
        //                            func = new Func<VoteItemInfo, bool>(class2, (IntPtr) this.<UpdateVoteAll>b__0);
        //                        }
        //                        if (Enumerable.Where<VoteItemInfo>(voteById.VoteItems, func).ToList<VoteItemInfo>().Count == 0)
        //                        {
        //                            this.CreateVoteItem(item);
        //                        }
        //                    }
        ...
        //            {
        //                CS$<>9__CachedAnonymousMethodDelegate3 = new Func<VoteItemInfo, string>(null, (IntPtr) <UpdateVoteAll>b__1);
        //            }
        //            List<string> nowId = Enumerable.Select<VoteItemInfo, string>(vote.VoteItems, CS$<>9__CachedAnonymousMethodDelegate3).ToList<string>();
        //            IEnumerable<VoteItemInfo> source = Enumerable.Where<VoteItemInfo>(voteById.VoteItems, new Func<VoteItemInfo, bool>(class3, (IntPtr) this.<UpdateVoteAll>b__2));
        //            if (source.Count<VoteItemInfo>() > 0)
        //            {
        //                foreach (VoteItemInfo info2 in source)
        //                {
