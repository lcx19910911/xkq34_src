namespace Hidistro.SqlDal.Promotions
{
    ...
    public class CouponDao
    {
        ...
        public bool AddCouponProducts(int couponId, bool IsAllProduct, IList<string> productIDs)
        {
            try
            {
                ...
                if (couponDetails != null)
                {
                    ...
                    if (IsAllProduct)
                    {
                        sqlStringCommand = this.database.GetSqlStringCommand("update Hishop_Coupon_Coupons set IsAllProduct=@IsAllProduct , ProductNumber=@ProductNumber WHERE CouponId = @CouponId");
                        this.database.AddInParameter(sqlStringCommand, "CouponId", DbType.Int32, couponId);
                        this.database.AddInParameter(sqlStringCommand, "ProductNumber", DbType.Int32, couponDetails.ProductNumber);
                    ...
                    str = str.Substring(1);
                    sqlStringCommand = this.database.GetSqlStringCommand(string.Format("Delete from  Hishop_Coupon_Products WHERE CouponId ={0} and ProductId in ( {1} )", couponId, str));
                    this.database.ExecuteNonQuery(sqlStringCommand);
                    StringBuilder builder = new StringBuilder();
        ...
        public DataTable GetUnFinishedCoupon(DateTime end, [Optional] CouponType? type)
        {
        ...
        public DataTable GetUserCoupons(int userId, [Optional, DefaultParameterValue(0)] int useType)
        {
            string str = "";
            if (useType == 1)
            {
