namespace Hidistro.SqlDal.Promotions
{
    ...
    public class ActivityDao
    {
        ...
        public bool AddActivitiesMember(int ActivitiesId, int Userid, [Optional, DefaultParameterValue(null)] DbTransaction dbTran)
        {
        ...
        public bool AddProducts(int actId, bool IsAllProduct, IList<string> productIDs)
        {
            try
            {
                ...
                if (act != null)
                {
                    ...
                    str = str.Substring(1);
                    sqlStringCommand = this.database.GetSqlStringCommand(string.Format("Delete from  Hishop_Activities_Product WHERE ActivitiesId ={0} and ProductId in ( {1} )", actId, str));
                    this.database.ExecuteNonQuery(sqlStringCommand);
                    StringBuilder builder = new StringBuilder();
        ...
        public ActivityInfo GetAct(int Id)
        {
            ...
            using (reader = this.database.ExecuteReader(sqlStringCommand))
            {
                IList<ActivityDetailInfo> list = ReaderConvert.ReaderToList<ActivityDetailInfo>(reader);
                info.Details = list;
            }
            return info;
        }
        ...
        public DataTable GetActivityTopics([Optional, DefaultParameterValue("0")] string types)
        {
        ...
        public int GetActivityTopicsNum([Optional, DefaultParameterValue("0")] string types)
        {
