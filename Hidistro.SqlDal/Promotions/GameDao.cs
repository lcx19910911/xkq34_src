namespace Hidistro.SqlDal.Promotions
{
    ...
    public class GameDao
    {
        ...
        {
            string query = "delete from Hishop_PromotionGameResultMembersLog where LogId in(select LogId from Hishop_PrizesDeliveryRecord where id in(" + string.Join<int>(",", ids) + "));delete from Hishop_PrizesDeliveryRecord where id in(" + string.Join<int>(",", ids) + ")";
            DbCommand sqlStringCommand = this._database.GetSqlStringCommand(query);
            return (this._database.ExecuteNonQuery(sqlStringCommand) > 0);
        }

        public DbQueryResult GetAllPrizesDeliveryList(PrizesDeliveQuery query, [Optional, DefaultParameterValue("")] string ExtendLimits, [Optional, DefaultParameterValue("*")] string selectFields)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append(" 1=1 ");
            if (query.Status >= 0)
        ...
        public DbQueryResult GetPrizesDeliveryList(PrizesDeliveQuery query, [Optional, DefaultParameterValue("")] string ExtendLimits, [Optional, DefaultParameterValue("*")] string selectFields)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append(" 1=1 ");
        ...
        public bool Update(GameInfo model, [Optional, DefaultParameterValue(true)] bool isNotStart)
        {
            bool flag;
            StringBuilder builder = new StringBuilder();
