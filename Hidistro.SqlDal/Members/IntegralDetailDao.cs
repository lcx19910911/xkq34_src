namespace Hidistro.SqlDal.Members
{
    ...
    public class IntegralDetailDao
    {
        ...
        public bool AddIntegralDetail(IntegralDetailInfo point, [Optional, DefaultParameterValue(null)] DbTransaction dbTran)
        {
        ...
        public DbQueryResult GetIntegralDetail(IntegralDetailQuery query)
        {
            ...
            if (query.StartTime.HasValue)
            {
                ...
                builder.AppendFormat("  convert(date,TrateTime) >='{0}'", DataHelper.GetSafeDateTimeFormat(query.StartTime.Value));
            }
            if (query.EndTime.HasValue)
            {
                ...
                builder.AppendFormat("   convert(date,TrateTime)<='{0}'", DataHelper.GetSafeDateTimeFormat(query.EndTime.Value));
            }
            if (query.UserId > 0)
            {
                if (builder.Length > 0)
