namespace Hidistro.SqlDal.Members
{
    ...
    public class MemberDao
    {
        ...
        public bool CheckCurrentMemberIsInRange(string Grades, string DefualtGroup, string CustomGroup, [Optional, DefaultParameterValue(0)] int tuserid)
        {
            bool flag = false;
        ...
        public bool CheckMemberIsBuyProds(int userId, string prodIds, DateTime? startTime, DateTime? endTime)
        {
            ...
            {
                obj3 = query;
                query = string.Concat(new object[] { obj3, " AND datediff(dd,'", endTime.Value.AddDays(1.0), "',o.PayDate) < 0" });
            }
        ...
        public bool ClearAllOpenId([Optional, DefaultParameterValue("wx")] string from)
        {
        ...
        public bool DelUserMessage(MemberInfo newuser, int olduserid)
        {
            ...
            obj2 = string.Concat(new object[] { obj2, " DELETE FROM aspnet_Members WHERE UserId =", newuser.UserId, "; " });
            obj2 = string.Concat(new object[] { obj2, " DELETE FROM Hishop_ShoppingCarts WHERE UserId =", newuser.UserId, "; " });
            obj2 = string.Concat(new object[] { obj2, " DELETE FROM Hishop_UserShippingAddresses WHERE UserId =", newuser.UserId, "; " });
            query = string.Concat(new object[] { obj2, " DELETE FROM vshop_ActivitySignUp WHERE UserId =", newuser.UserId, "; " }) + " Update  aspnet_Members ";
            string str2 = "";
            ...
            if (!string.IsNullOrEmpty(newuser.AlipayOpenid))
            {
                ...
                string str3 = str2;
                str3 = str3 + "AlipayUserId='" + newuser.AlipayUserId + "',AlipayOpenid='" + newuser.AlipayOpenid + "'";
                str2 = (str3 + ",AlipayLoginId='" + newuser.AlipayLoginId + "',AlipayUsername='" + newuser.AlipayUsername + "'") + ",AlipayAvatar='" + newuser.AlipayAvatar + "' ";
            ...
            obj2 = query;
            query = string.Concat(new object[] { obj2, str2, " WHERE UserId =", olduserid, " ;" }) + " COMMIT TRAN TranUpdate" + "  end try \r\n                    begin catch \r\n                        ROLLBACK TRAN TranUpdate\r\n                    end catch ";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
            return (this.database.ExecuteNonQuery(sqlStringCommand) > 0);
        }
        ...
        public bool DelUserMessage(int userid, string openid, string userhead, int olduserid)
        {
            string query = "";
            object obj2 = query + "begin try  " + "  begin tran TranUpdate";
            obj2 = string.Concat(new object[] { obj2, " DELETE FROM aspnet_Members WHERE UserId =", userid, "; " });
            obj2 = string.Concat(new object[] { obj2, " DELETE FROM Hishop_ShoppingCarts WHERE UserId =", userid, "; " });
            obj2 = string.Concat(new object[] { obj2, " DELETE FROM Hishop_UserShippingAddresses WHERE UserId =", userid, "; " });
            obj2 = string.Concat(new object[] { obj2, " DELETE FROM vshop_ActivitySignUp WHERE UserId =", userid, "; " });
            query = string.Concat(new object[] { obj2, " Update  aspnet_Members set OpenId='", openid, "',UserHead='", userhead, "' WHERE UserId =", olduserid, "; " }) + " COMMIT TRAN TranUpdate" + "  end try \r\n                    begin catch \r\n                        ROLLBACK TRAN TranUpdate\r\n                    end catch ";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
            return (this.database.ExecuteNonQuery(sqlStringCommand) > 0);
        }
        ...
        public IList<int> GetCreateDateIntervalMemberList(DateTime? start, DateTime? end)
        {
            ...
            {
                obj2 = query;
                query = string.Concat(new object[] { obj2, " AND datediff(dd,'", end.Value, "',CreateDate) <= 0" });
            }
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
        ...
        public IList<int> GetDefualtGroupMemberList(string DefualtGroup)
        {
            ...
            if (DefualtGroup.Contains("2"))
            {
                ...
                else
                {
                    ...
                    foreach (int num in activyGroupMemberList)
                    {
            ...
            if (DefualtGroup.Contains("3"))
            {
                ...
                foreach (int num in sleepGroupMemberList)
                {
                    newGroupMemberList.Add(num);
                }
            }
        ...
        public int GetMemberIdByUserNameOrNiChen([Optional, DefaultParameterValue("")] string username, [Optional, DefaultParameterValue("")] string nich)
        {
            DbCommand sqlStringCommand;
            object obj2;
            if (!string.IsNullOrWhiteSpace(username))
            {
                sqlStringCommand = this.database.GetSqlStringCommand("SELECT UserId FROM aspnet_Members WHERE UserBindName = @UserBindName AND Status=@Status");
            ...
            {
                sqlStringCommand = this.database.GetSqlStringCommand("SELECT UserId FROM aspnet_Members WHERE UserName = @UserName AND Status=@Status");
                this.database.AddInParameter(sqlStringCommand, "UserName", DbType.String, nich);
                this.database.AddInParameter(sqlStringCommand, "Status", DbType.Int32, Convert.ToInt32(UserStatus.Normal));
                obj2 = this.database.ExecuteScalar(sqlStringCommand);
                if (obj2 != null)
                {
        ...
        public DbQueryResult GetMembers(MemberQuery query, [Optional, DefaultParameterValue(false)] bool isNotBindUserName)
        {
            ...
            if (!string.IsNullOrEmpty(query.StoreName))
            {
                ...
                {
                    builder.AppendFormat(" AND ReferralUserId not in  (select userid from  dbo.aspnet_Distributors where  UserId=m.ReferralUserId)", new object[0]);
                }
                else
                {
                    builder.AppendFormat(" AND ReferralUserId in  (SELECT UserId  FROM aspnet_Distributors where storename='{0}' )", query.StoreName);
                }
            }
            ...
            if (!string.IsNullOrEmpty(query.ClientType))
            {
                ...
                if (!(clientType == "new"))
                {
                    ...
                    {
                        num2 = new MemberDao().GetActiveDay();
                        builder.AppendFormat("  and PayOrderDate is not null and PayOrderDate >='" + DateTime.Now.AddDays((double) -num2).ToString("yyyy-MM-dd HH:mm:ss") + "' ", new object[0]);
                        goto Label_0455;
                    }
                    if (clientType == "sleep")
                    {
                        num2 = new MemberDao().GetActiveDay();
                        builder.AppendFormat("  and  (PayOrderDate is null or PayOrderDate <'" + DateTime.Now.AddDays((double) -num2).ToString("yyyy-MM-dd HH:mm:ss") + "') ", new object[0]);
                        goto Label_0455;
                    }
                    goto Label_0440;
                }
        ...
        public DataTable GetMembersByUserId(int referralUserId, int pageIndex, int pageSize, out int total)
        {
            DataTable table;
            total = 0;
            ...
            string str2 = string.Format(" SELECT * FROM (select ROW_NUMBER()  OVER (ORDER BY s.userid) as [RowIndex] , s.userid,username,(SELECT Name FROM aspnet_MemberGrades WHERE GradeId = s.GradeId) AS GradeName,UserHead,CreateDate,s.ReferralUserId,d.StoreName, (select count(*) from Hishop_Orders where UserId=s.UserId) as OrderMumber, (select top 1 OrderDate from Hishop_Orders where UserId=s.UserId order by OrderDate desc) as OrderDate, (select SUM(OrderTotal) from Hishop_Orders where UserId=s.UserId )as OrdersTotal  from aspnet_Members s left join aspnet_Distributors d on s.userid=d.userid  where  s.ReferralUserId={0} and Status=1 and d.StoreName is null ) AS W WHERE  RowIndex between {1} and {2}", referralUserId, num, num2);
        ...
        public MemberInfo GetOpenIdMember(string openId, [Optional, DefaultParameterValue("wx")] string From)
        {
        ...
        public IList<int> GetPayDateIntervalMemberList(DateTime? start, DateTime? end)
        {
            ...
            {
                obj2 = query;
                query = string.Concat(new object[] { obj2, " AND datediff(dd,'", end.Value, "',v.PayDate) <= 0" });
            }
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
        ...
        public int GetUserFollowStateByUserId(int UserId, [Optional, DefaultParameterValue("wx")] string type)
        {
            int num = 0;
            string query = "";
            if (type == "wx")
        ...
        public bool ReSetUserHead(string userid, string wxName, string wxHead, [Optional, DefaultParameterValue("")] string Openid)
        {
            string str = "";
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("UPDATE  aspnet_Members SET UserName = @UserName,UserHead=@UserHead,IsAuthorizeWeiXin=1 where OpenId = @OpenId ");
            this.database.AddInParameter(sqlStringCommand, "OpenId", DbType.String, OpenId);
        ...
        public bool UpdateUserFollowStateByUserId(int UserId, int state, [Optional, DefaultParameterValue("wx")] string type)
        {
            string query = "";
            if (type == "wx")
            {
