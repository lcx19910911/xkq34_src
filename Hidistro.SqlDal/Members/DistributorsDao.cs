namespace Hidistro.SqlDal.Members
{
    ...
    public class DistributorsDao
    {
        ...
        public DbQueryResult GetBalanceDrawRequest(BalanceDrawRequestQuery query, [Optional] string[] extendCheckStatus)
        {
            StringBuilder builder = new StringBuilder();
        ...
        public DbQueryResult GetCommissionsWithStoreName(CommissionsQuery query, [Optional, DefaultParameterValue("")] string level)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append(" State=1 ");
            if (query.UserId > 0)
            ...
            if (!string.IsNullOrEmpty(level))
            {
                ...
                else if (level == "0")
                {
                    if (!string.IsNullOrEmpty(query.StoreName))
                    {
                        builder.Append(" and CommType<>3  and CommType<>5 ");
                    }
                    else
                    {
        ...
        public DbQueryResult GetDistributors(DistributorsQuery query, [Optional, DefaultParameterValue(null)] string TopUserId, [Optional, DefaultParameterValue(null)] string level)
        {
            StringBuilder builder = new StringBuilder();
            if (query.GradeId > 0)
            {
        ...
        {
            StringBuilder builder = new StringBuilder();
            builder.AppendFormat("Select (SELECT count(UserId) FROM vw_Hishop_DistributorsMembers where ReferralStatus!=9 and (ReferralPath='{0}' or ReferralPath like '%|{0}')) firstV,(SELECT count(UserId) FROM  vw_Hishop_DistributorsMembers where ReferralStatus!=9 and   ReferralPath like '{0}|%') secondV ,( select count(*) from aspnet_Members  s left join aspnet_Distributors d on s.userid=d.userid  where  s.Status=1 and s.ReferralUserId={0} and d.StoreName is null) memberCount", topUserId);
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(builder.ToString());
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                return DataHelper.ConverDataReaderToDataTable(reader);
        ...
        public DataTable GetDownDistributors(DistributorsQuery query, out int total)
        {
            StringBuilder builder = new StringBuilder(" ReferralStatus!=9 ");
            string str = "";
            if (query.GradeId > 0)
            {
                if (query.GradeId == 2)
        ...
        public decimal GetUserCommissions(int userid, DateTime fromdatetime, [Optional, DefaultParameterValue(null)] string enddatetime, [Optional, DefaultParameterValue(null)] string storeName, [Optional, DefaultParameterValue(null)] string OrderNum, [Optional, DefaultParameterValue("")] string level)
        {
            DateTime time;
            StringBuilder builder = new StringBuilder();
            builder.Append(" State=1 ");
        ...
        public DataTable SelectDistributors(DistributorsQuery query, [Optional, DefaultParameterValue(null)] string TopUserId, [Optional, DefaultParameterValue(null)] string level)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append(" 1=1 ");
            if (!string.IsNullOrEmpty(query.StoreName))
        ...
        public bool SetBalanceDrawRequestIsCheckStatus(int[] serialid, int checkValue, [Optional, DefaultParameterValue(null)] string Remark, [Optional, DefaultParameterValue(null)] string Amount)
        {
            ...
            query = query + " where SerialID in(" + string.Join<int>(",", serialid) + ")";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
        ...
        public bool UpdateBalanceDrawRequest(int Id, string Remark, [Optional, DefaultParameterValue(null)] string CheckTime)
        {
            if (CheckTime == null)
            {
